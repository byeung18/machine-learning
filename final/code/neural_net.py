...
class NeuralNet():
    ...
    def funObj(self, weights_flat, X, y):
        ...
        for i in range(len(self.layer_sizes)-2,0,-1):
            W, b = weights[i]
            grad = grad @ W
            grad = grad * (activations[i] > 0)      # ReLU gradient
    ...
    def fit(self, X, y):
        ...
        weights_flat_new, f = findMin(self.funObj, weights_flat,  X, y, rate=self.learning_rate, maxEvals=self.max_iter, verbose=True)
...
def findMin(funObj, w, *args, rate, maxEvals, verbose=0):
    ...
    f, g = funObj(w, X[:minibatch_size], y[:minibatch_size])
    alpha = rate
    count = 1
    # previous_loss = 99999
    while True:
        ...
        if count % 100 == 0:
            alpha = alpha / 2
        for batch in range(n // minibatch_size):
            start = batch * minibatch_size
            end = start + minibatch_size
            X_batch = X[start:end]
            y_batch = y[start:end]
            w_new = w - alpha * g
            f_new, g_new = funObj(w_new, X_batch, y_batch)
        ...
        if verbose > 0:
            # if count % 50 == 0:
            print("%d - loss: %.3f" % (count, f_new))

            # if previous_loss - f_new < 0.001:
            #     break
            # previous_loss = f_new
